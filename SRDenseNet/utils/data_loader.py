...
from SRDenseNet.utils.other_utils import *
import time
...
class Dataset_Loader():
    ...
    def bicubic_down(self, img, scale=3):
        # numpy
        img = denormalize(img)
    ...
    def fn_to_single_image_pair(self, img_fn, use_crop = True):
        img = img_fn_to_img(img_fn)
        img = normalize(img)
        if use_crop:
            seed = time.time()
            img = random_crop(img, patch_size=self.patch_size, seed=seed)

        img = self.mod_crop(img, self.config.scale)
        label = img
        input = self.bicubic_down(img, scale = self.scale)
    ...
    def get_random_testset(self):
        ...
        input, label = self.fn_to_single_image_pair(self.labels_fn_test[rand_idx], use_crop = False)
    ...
    def get_testset(self):
        ...
        for i in range(len(self.labels_fn_test)) :
            #pick
            input, label = self.fn_to_single_image_pair(self.labels_fn_test[i], use_crop = False)
...
if __name__ == "__main__":
    ...
    for input,label in data_loader.get_testset():
        ...
        input = input[0]
        label = label[0]
        input = np.pad(input, ((0, label.shape[0] - input.shape[0]), (0, label.shape[1] - input.shape[1]), (0, 0)))
        cv2.imshow("image",np.concatenate([input,label],axis=1)[...,::-1])
        cv2.waitKey(0)
