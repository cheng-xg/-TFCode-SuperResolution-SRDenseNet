...
if config.exp_type == 0:
    from SRDenseNet.utils.data_loader import *
    from models.model_SRDenseNet import Model_Train
    config.model_tag = "SRDenseNet_model_default"

def generate_expname_automatically():
    name = "SRDenseNet_%s_%02d_%02d_%02d_%02d_%02d" % (config.model_tag,
...
while True : #manuallry stopping
    ...
    if step % 1 == 0:
        ...
        result_concat = np.concatenate([output,label],axis=2)
    ...
    if step % 10 == 0:
        ...
        result_concat = np.concatenate([output_test,label_test],axis=2)
