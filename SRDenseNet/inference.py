...
parser.add_argument("--restore_model_file", type=str, default='../__outputs/checkpoints/VDSR_1010/model.ckpt-31000', help='file for resotration')
...
if config.exp_type == 0:
    ...
    from models.inference_SRDenseNet import Model_Inference
...
while True : #manuallry stopping
    ...
    img_up_tensor = normalize(np.expand_dims(img_up,0)[...,0:config.num_channels])
    log, elapse, output, _ = model.inference(img_up_tensor)

    print(log)
    elapses.append(elapse)

    img_pad = np.pad(img, ((0,img_up.shape[0] - img.shape[0] ),(0,img_up.shape[1] - img.shape[1]), (0,0))  )
    output_concat = np.concatenate([img_pad,img_up,output[0]],axis=1)
    cv2.imshow('result_test_{}_{}'.format(config.exp_type, config.model_tag), resize(output_concat[..., ::-1],1))
    cv2.waitKey(10)
